...
class SEIR_SDE_Simulator(torch.autograd.Function):
    ...
    def forward(ctx, inputs, batch_data, device):
        ...
        y = batch_data["ys"][1:-1][nearest, range(nearest.shape[0])].reshape(-1, 4)
        y = y[:, 2].reshape(-1, 1)  # Focus on Infected compartment
        
        ctx.save_for_backward(inputs)
        ctx.device = device
        ctx.nearest = nearest
        ctx.grads = (batch_data["ys"][2:, :] - batch_data["ys"][:-2, :]) / (2 * batch_data["dt"])
    ...
    def backward(ctx, grad_output):
        ...
        y_grads = y_grads[:, 2].reshape(-1, 1)  # Focus on gradient of Infected compartment

        Jac = y_grads.unsqueeze(-1)  # Shape: (batch_size, 1, 1)

        grad_output = grad_output.reshape(Jac.shape[0], Jac.shape[1], -1)

        grad_input = Jac * grad_output  # Element-wise multiplication
...
class Epidemic(nn.Module):
    ...
    def __init__(
        ...
        self.p = 3  # Assuming 3 parameters here
    ...
    def theta_to_index(self, theta):
        if self.SIMDATA is None:
            raise ValueError("SIMDATA is not initialized.")
        if "prior_samples" not in self.SIMDATA or self.SIMDATA["prior_samples"] is None:
            raise ValueError("SIMDATA['prior_samples'] is not initialized.")

        if theta.shape[-1] != self.SIMDATA["prior_samples"].shape[-1]:
            raise ValueError(f"Dimension mismatch: theta has size {theta.shape[-1]} but prior_samples has size {self.SIMDATA['prior_samples'].shape[-1]}")

        theta_expanded = theta.unsqueeze(1).expand(
            theta.shape[0], self.SIMDATA["prior_samples"].shape[0], self.SIMDATA["prior_samples"].shape[-1]
        )
...
def train_model(
    ...
    n = 1
    design_dim = (n, 1)
    latent_dim = 3  # Updated to match the prior_samples
    observation_dim = n
    ...
    if design_arch == "sum":
        ...
        design_net = ConstantBatchBaseline(design_dim=design_dim, const_designs_list=const_designs).to(device)
    ...
    critic_latent_encoder = Mlp(
        input_dim=latent_dim,  # Adjusted to match the actual input dimensions
    ...
    test_theta = torch.tensor([[0.60, 0.15, 0.05]], dtype=torch.float, device=device)  # Ensure it has 3 parameters
    ...
    if not os.path.exists("mlflow_outputs_seir"):
        os.makedirs("mlflow_outputs_seir")

    pd.concat(outputs_history).reset_index().to_csv(f"mlflow_outputs_seir/designs_hist.csv")
    mlflow.log_artifact(f"mlflow_outputs_seir/designs_hist.csv", artifact_path="designs")
    ...
    print(f"Model and critic stored in {model_loc}. Done.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mi-estimator", default="NWJ", type=str)
    parser.add_argument(
